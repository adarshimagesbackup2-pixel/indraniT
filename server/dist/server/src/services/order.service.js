...
exports.setEwayBillNumber = setEwayBillNumber;
 ...
 */
function splitGst(taxableAmount, gstRatePercent, isIntrastate) {
...
async function createOrder({ input, createdById, isAdmin }) {
    return prisma_1.prisma.$transaction(async (tx) => {
        ...
        if (!customer.stateCode || !customer.pincode) {
            throw new errorHandler_1.ApiError(422, `${customer.name} is missing their GST state code and/or pincode. Add these in Masters → Customers before creating an order for them.`);
        }
        // §4 — blacklist hard-block, ADMIN-only override (same pattern as credit limit)
        if (customer.isBlacklisted) {
            if (!input.overrideBlacklist || !isAdmin) {
                throw new errorHandler_1.ApiError(422, `This customer is blacklisted: ${customer.blacklistReason}. Contact an admin to remove this status before creating new orders.`);
            }
        }
        const transport = await tx.transport.findUnique({ where: { id: input.transportId } });
        if (!transport || !transport.isActive) {
            throw new errorHandler_1.ApiError(422, "Transport vehicle not found or inactive");
        }
        ...
        const requestedQtyByBag = new Map();
        ...
        for (const [bagTypeId, requestedQty] of requestedQtyByBag) {
            ...
            if (!bag)
                throw new errorHandler_1.ApiError(422, "One or more bag types were not found");
            if (requestedQty > bag.currentStock) {
                throw new errorHandler_1.ApiError(422, `Only ${bag.currentStock} of "${bag.bagType}" available in stock (requested ${requestedQty})`, { [`items.${bagTypeId}`]: `Only ${bag.currentStock} available` });
            }
        }
        ...
        const lineItemsData = input.items.map((item) => {
            ...
            const lineTotal = item.pricingType === "PER_BAG"
        ...
        const totalAmount = (0, currency_1.roundToRupee)(subtotal + totalCgst + totalSgst + totalIgst);
        const currentOutstanding = await (0, khata_service_1.getLatestRunningBalance)(customer.id, tx);
        ...
        if (creditLimit > 0 && newBalance > creditLimit) {
            if (!input.overrideCreditLimit || !isAdmin) {
                ...
                throw new errorHandler_1.ApiError(422, `This order exceeds the customer's credit limit by ₹${overage.toFixed(2)}. Current outstanding: ₹${currentOutstanding.toFixed(2)}, Credit limit: ₹${creditLimit.toFixed(2)}.`);
            }
        ...
        if (input.customChallanNo && input.customChallanNo.trim().length > 0) {
            ...
            if (clash) {
                throw new errorHandler_1.ApiError(422, "This number is already in use", { customChallanNo: "This number is already in use" });
            }
            challanNo = input.customChallanNo.trim();
        }
        else {
            if (settings.numberingMode === "MANUAL") {
                throw new errorHandler_1.ApiError(422, "Numbering Mode is set to Manual — enter a bill/challan number.", {
                    customChallanNo: "A number is required in Manual numbering mode",
                });
            }
            challanNo = await (0, challanNumber_1.generateChallanNumber)(tx);
        }
        ...
        const order = await tx.order.create({
            data: {
                ...
                transportDocDate: input.transportMode === "ROAD" || !input.transportDocDate ? null : new Date(input.transportDocDate),
...
async function setEwayBillNumber(orderId, ewayBillNo) {
    const order = await prisma_1.prisma.order.findUnique({ where: { id: orderId } });
    if (!order)
        throw new errorHandler_1.ApiError(404, "Order not found");
    if (order.ewayBillStatus === "NOT_REQUIRED") {
        throw new errorHandler_1.ApiError(422, "This order does not require an e-Way Bill");
    }
    return prisma_1.prisma.order.update({ where: { id: orderId }, data: { ewayBillNo, ewayBillStatus: "GENERATED" } });
}
async function listOrders(filters) {
    ...
    if (filters.vehicleId)
        where.transportId = filters.vehicleId;
    if (filters.ewayStatus)
        where.ewayBillStatus = filters.ewayStatus;
    if (filters.search) {
        where.OR = [
 ...
 */
async function editOrder(orderId, input, editedById) {
    return prisma_1.prisma.$transaction(async (tx) => {
        ...
        if (existing.status === "CANCELLED")
            throw new errorHandler_1.ApiError(422, "This order has been cancelled and cannot be edited");
        if (existing.ewayBillStatus === "GENERATED") {
            throw new errorHandler_1.ApiError(422, "This order already has an e-Way Bill generated. Cancel/update the e-Way Bill on the government portal first, or create a credit note instead.");
        }
        const settings = await tx.businessSettings.findUniqueOrThrow({ where: { id: "singleton" } });
        const customer = await tx.customer.findUnique({ where: { id: input.customerId } });
        if (!customer)
            throw new errorHandler_1.ApiError(422, "Customer not found");
        if (!customer.stateCode || !customer.pincode) {
            throw new errorHandler_1.ApiError(422, `${customer.name} is missing their GST state code and/or pincode.`);
        }
        const transport = await tx.transport.findUnique({ where: { id: input.transportId } });
        if (!transport)
            throw new errorHandler_1.ApiError(422, "Transport vehicle not found");
        ...
        for (const [bagTypeId, requestedQty] of requestedQtyByBag) {
            ...
            if (!bag)
                throw new errorHandler_1.ApiError(422, "One or more bag types were not found");
            if (requestedQty > bag.currentStock) {
                throw new errorHandler_1.ApiError(422, `Only ${bag.currentStock} of "${bag.bagType}" available in stock (requested ${requestedQty})`);
            }
        }
        const isIntrastate = customer.stateCode === settings.businessStateCode;
        let subtotal = 0;
        ...
        const newTotalAmount = (0, currency_1.roundToRupee)(subtotal + totalCgst + totalSgst + totalIgst);
        for (const [bagTypeId, requestedQty] of requestedQtyByBag) {
        ...
        const balanceBeforeReversal = await (0, khata_service_1.getLatestRunningBalance)(existing.customerId, tx);
        await tx.khataLedger.create({
            data: {
        ...
        await (0, khata_service_1.recalculateBalances)(existing.customerId, tx);
        if (input.customerId !== existing.customerId) {
            await (0, khata_service_1.recalculateBalances)(input.customerId, tx);
        }
 ...
 */
async function cancelOrder(orderId, cancelReason, cancelledById) {
    return prisma_1.prisma.$transaction(async (tx) => {
        const existing = await tx.order.findUnique({ where: { id: orderId }, include: { items: true } });
        if (!existing)
            throw new errorHandler_1.ApiError(404, "Order not found");
        if (existing.status === "CANCELLED")
            throw new errorHandler_1.ApiError(422, "This order is already cancelled");
        ...
        const originalLedgerEntry = await tx.khataLedger.findFirst({ where: { orderId: existing.id, isCorrection: false } });
        const balanceBefore = await (0, khata_service_1.getLatestRunningBalance)(existing.customerId, tx);
        const oldTotal = Number(existing.totalAmount);
        await tx.khataLedger.create({
            data: {
                customerId: existing.customerId,
