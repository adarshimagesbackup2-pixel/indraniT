...
import { SensitiveAmount } from "../components/ui/SensitiveAmount";
      ...
      <div className="no-print grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <div className="mt-1 text-2xl font-bold text-danger">
            <SensitiveAmount value={formatCurrency(data.totalOutstanding)} />
          </div>
        </Card>
        <Card>
          ...
          <div className="text-xs text-slate-500">
            <SensitiveAmount value={formatCurrency(data.todaysOrdersValue)} className="text-xs" />
          </div>
        </Card>
        <Card>
          <div className="text-sm text-slate-500">This Month's Collections</div>
          <div className="mt-1 text-2xl font-bold text-success">
            <SensitiveAmount value={formatCurrency(data.monthCollections)} />
          </div>
        </Card>
        <Card>
            ...
            <ul className="space-y-1 text-sm">
              {data.recentActivity.orders.map((o) => (
                <li key={o.id} className="flex justify-between">
                  ...
                  <span className="mono">
                    <SensitiveAmount value={formatCurrency(o.totalAmount)} className="text-sm" />
                  </span>
                </li>
              ))}
            </ul>
              ...
              {data.recentActivity.payments.map((p) => (
                <li key={p.id} className="flex justify-between">
                  ...
                  <span className="mono text-success">
                    <SensitiveAmount value={formatCurrency(p.amount)} className="text-sm text-success" />
                  </span>
                </li>
              ))}
            </ul>
