      ...
      <div className="no-print grid grid-cols-1 gap-4 sm:grid-cols-3">
        <Card>
          ...
          <div className={`mt-1 text-2xl font-bold ${balance.isAdvance ? "text-success" : "text-danger"}`}>
            <SensitiveAmount value={balance.label} />
          </div>
        </Card>
        <Card>
          <div className="text-sm text-slate-500">Credit Limit</div>
          <div className="mt-1 text-2xl font-bold">
            {data.customer.creditLimit > 0 ? <SensitiveAmount value={formatCurrency(data.customer.creditLimit)} /> : "Unlimited"}
          </div>
        </Card>
        <Card>
          ...
          <div className="mt-1 text-lg font-semibold">
            {data.lastPaymentDate ? (
              <SensitiveAmount value={`${formatCurrency(data.lastPaymentAmount ?? 0)} on ${formatDate(data.lastPaymentDate)}`} />
            ) : (
              "No payments yet"
            )}
          </div>
